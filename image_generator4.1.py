# -*- coding: utf-8 -*-
# TODO: tensorflow の API で [(x, y, r, g, b)] を作りたい
...
w = tf.placeholder(tf.int32, shape=(None,))
h = tf.placeholder(tf.int32, shape=(None,))

xs = tf.range(w) / (w - 1) - 0.5
ys = tf.range(h) / (h - 1) - 0.5

tf.tile(tf.expand_dims(xs, 0), [h, 1])
tf.tile(tf.expand_dims(xs, 1), [1, w])
